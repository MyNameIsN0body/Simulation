...
public class BFSPathfinder1 {
    ...
    private static boolean isPassable(WorldMap worldMap, int x, int y, String entityType) {
        ...
        Optional<Entity> entity = worldMap.getEntity(new Coordinates(x, y));
        if (entity.isEmpty()) {
            return true;
        }
        ...
        switch (entityType) {
            case "HERBIVORE":
                ...
                return entity.get().getType() == EntityType.GRASS ||
                        entity.get().getType() == EntityType.HERBIVORE;
            case "PREDATOR":
                // Хищники могут проходить через траву и травоядных
                return entity.get().getType() == EntityType.GRASS ||
                        entity.get().getType() == EntityType.HERBIVORE||
                        entity.get().getType() == EntityType.PREDATOR;
            default:
